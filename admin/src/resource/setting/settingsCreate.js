      ...
      for (const item of values.data) {
          if (item.withFormula === 'is') {
              if (!item.staticPrice || !item.perKiloPrice) {
                  ...
                  notify(translate("static or per kilo price is not entered"), {
                    type: "error"
                  });
    ...
    <SimpleForm {...props} onSubmit={save}>
      ...
      <ArrayInput source="data">
        ...
        />
          ...
          />
          <SelectInput
              label={translate("resources.settings.withFormula_withoutFormula")}
              fullWidth
              className={"mb-20"}
              source="withFormula"
          ...
          <TextInput
              ...
              fullWidth
              source="staticPrice"
              label={translate("resources.settings.staticPrice")}
          />
          <TextInput
              fullWidth
              type='number'
              source="perKiloPrice"
              label={translate("resources.settings.perKiloPrice")}
          />      
