...
function onCreateCombinations(options) {
    ...
    // (id, path, rowRecord) => test.change('combinations', combinations)
...
const Form = ({children, ...props}) => {
    ...
    const transtest = (data, {previousData}) => {
        console.log("transtest={transtest}", data, {previousData})
    ...
    return (
        ...
        >
            {/*<TabbedDatagrid/>*/}
            <TextInput source={"title."+translate("lan")} fullWidth label={translate("resources.test.title")}
                       className={"width100 mb-20"}
                       validate={Val.req}/>

            <div className={"mb-20"}></div>

            <TextInput source="slug" fullWidth label={translate("resources.test.slug")}
                       className={"width100 mb-20 ltr"}/>
            <TextInput source="classes" fullWidth label={translate("resources.test.classes")}
                       className={"width100 mb-20 ltr"}/>

            <TextInput source="sort" fullWidth label={translate("resources.test.sort")}
                       className={"width100 mb-20 ltr"}/>


            {/*<SelectInput*/}
                {/*label={translate("resources.test.kind")}*/}
                {/*defaultValue={"test"}*/}
                {/*source="kind"*/}
                {/*choices={[*/}
                    {/*{id: "test", name: translate("resources.test.test")},*/}
                    {/*{id: "page", name: translate("resources.test.page")}*/}
                {/*]}*/}
            {/*/>*/}
            <ReferenceInput
