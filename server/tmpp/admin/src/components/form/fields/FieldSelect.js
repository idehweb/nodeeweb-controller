...
function FieldSelect(props) {
  ...
  useEffect(() => {
    ...
    if(typeInitila === 'testCategory'){
      API.get(BASE_URL + `/admin/testCategory/0/${limit}?_order=ASC&_sort=id`).then((res)=>{
        setList(res.data)
      }).catch((err)=>{
  ...
  return <Col
    ...
    >
      <option/>
        {/*<option>{typeInitila}</option>*/}
        {
          (typeInitila === 'form' || typeInitila==='testCategory') ? (
          list && list.map((ch, c) => {
            return <option key={c} value={ch._id} style={{fontFamily:'IRANSans !importan'}}>
              {ch?.slug} | {ch?.title?.fa} | {ch?.name?.fa}
            </option>
          })
        ):(
          list && list.map((ch, c) => {
