...
const GetCountOfNeverBought = (props) => {
    const { icon='Person4', title='کاربرانی که تابحال خرید نداشتند', subtitle='', to='', children } = props;
    const classes = useStyles(props);
    const [theData,setTheData]=useState(0);
    const getShopData = () => {
        API.get("/settings/get-count-of-never-bought").then(({ data = {} }) => {
            // setLoading(false);
...
export default GetCountOfNeverBought;
