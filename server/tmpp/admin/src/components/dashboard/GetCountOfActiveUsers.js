...
const GetCountOfActiveUsers = (props) => {
    const { icon='Person4', title='انقضاشون بیشتر از ۱۴ روز مونده', subtitle='', to='', children } = props;
    const classes = useStyles(props);
    const [theData,setTheData]=useState(0);
    const getShopData = () => {
        API.get("/settings/get-active-user").then(({ data = {} }) => {
            // setLoading(false);
...
export default GetCountOfActiveUsers;
