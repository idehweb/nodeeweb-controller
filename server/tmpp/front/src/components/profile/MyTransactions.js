...
class MyTransactions extends React.Component {
    ...
    getMyTransactions().then((data) => {
      if (data && data.length > 0)
        data.map((post) => {
          ...
          if (post && post['amount']) {
            console.log('hihihihihihih transaction', post.order['amount'])
            post['amount'] =
          ...
          if (post && post['sum']) {
            ...
            if (post.order && post.order['amount']) {
              console.log('hihihihihihih transaction')
              post.order['amount'] =
                post.order['amount']
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ',') + t(' UZS');
            }
            if (post.order && post.order['deliveryPrice']) {
              post.order['deliveryPrice'] =
                post.order['deliveryPrice']
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ',') + t(' UZS');
            }
  ...
  render() {
    let { data, headCells, newText, buttonText, paymentStatusLabel } = this.state;
    let { t } = this.props;
    // if (redirect) {
                              ...
                              <div className={'the-order-title'}>
                                ...
                                <div className={'the-order-body-line'}>
                                  ...
                                  <div className={'the-order-body-line'}>
                                    <Link
                                      className={'gfdsdf'}
                                      to={'/order-details/' + dat.order?._id}>
                                      <OpenInNewIcon />
                                      {t('view items')}
                                    </Link>
                                  </div>
